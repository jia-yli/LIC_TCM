...
def main():
  ...
  for variable in variable_lst:
    for model_name in models:
      for quality_factor in quality_factors[model_name]:
        print(f"[INFO] Starting {model_name} eval on {variable} with quality factor {quality_factor} ......")
        eval_configs = Config({
        ...
        if num_gpus > 1:
          eval_metrics = pool.apply_async(eval_zoo_era5, args = (eval_configs,))
        else:
          eval_metrics = eval_zoo_era5(eval_configs)
        result = {
          "variable": variable,
  ...
  for idx in range(len(results)):
    ...
    print(f"[INFO] Waiting {async_result['model_name']} eval on {async_result['variable']}  with quality factor {async_result['quality_factor']} ......")
    if num_gpus > 1:
      result = {
    ...
    df.to_csv("./results/eval_zoo_era5.csv", index=False)
